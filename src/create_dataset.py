...
def populate_train_list(images_path, train=True):
    ...
    for i in range(60,74):
        ...
        if os.path.isdir(os.path.join(images_path, folder)):
            file1 = open(os.path.join(images_path, folder, 'input_exp.txt'), 'r') 
            ...
            for m in range(3):
                for n in range(5):
                    ...
                    batch_np_rt = np.zeros([4, 256, 256, 6])
                    ...
                    f = os.path.join(images_path, folder, 'ref_hdr.hdr')
                    hdr = (cv2.imread(f, -1)).astype(np.float32)

                    hdr = hdr[a:a+256, b:b+256]
...
populate_train_list('dataset/', True)